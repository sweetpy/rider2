...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['update'])) {
    ...
    $result = Payments::movePaymentAmount(
        $payment_id,
        $rental_id,
        $amount_to_move,
        $payment_date,
    ...
    if (strpos($result, 'Error') === false) {
        ...
        // Send recon email to support
        $mail_to = "support@flit.tz";
        $mail_subject = "Payment Update Notification: Payment ID $payment_id";
        $mail_body = "
Dear Support Team,

This is to notify you that a payment record has been updated.

Details:
- Payment ID: $payment_id
- Rental ID: $rental_id
- Amount Moved: $amount_to_move
- Original Payment Date: $payment_date
- New Payment Date: $target_payment_date
- Updated By: {$_SESSION['username']}
- Timestamp: " . date('Y-m-d H:i:s') . "

Please review this action at your earliest convenience.

Best regards,
Rider Console.";
        $mails_to_cc = array("edgar@flit.tz");
        send_mail($mail_to, $mail_subject, $mail_body, $mails_to_cc);

    } else {
        $error_message = 'Error: Failed to update payment record. Please try again.';
    }
}
                            ...
                            <form method="post">
                                <div class="card-body">
                                    ...
                                    <div class="row g-3">
                                        ...
                                        <!-- Banner to display excess amount -->
                                        <div class="col-12">
                                            <div class="alert alert-warning" style="opacity: 1">
                                                <h6 class="alert-heading">Excess Payment</h6>
                                                <?php $excess = $amount_paid - $daily_rental_fee; ?>
                                                <p class="mb-0">There is excess:
                                                    <?php echo htmlspecialchars($excess); ?> on this
                                                    payment. Please Select a target date to update.
                                                </p>
                                            </div>
                                        </div>
                                    ...
                                    <div class="card-footer d-flex justify-content-between">
                                        ...
                                        <button type="submit" name="update" class="btn btn-primary">Fill
                                            Missing</button>
                                    </div>
                                </div>
                            </form>
